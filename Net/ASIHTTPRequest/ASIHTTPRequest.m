...
- (void)main
{
	@try {
		...
		if ([self downloadCache]) {
			...
			if (([self cachePolicy] & ASIAskServerIfModifiedCachePolicy)||([self cachePolicy]&ASIFallbackToCacheIfLoadFailsCachePolicy)){ 
                //NSLog(@"ASIAskServerIfModifiedCachePolicy|ASIFallbackToCacheIfLoadFailsCachePolicy");
            }else if ([[self downloadCache] canUseCachedDataForRequest:self]) {
